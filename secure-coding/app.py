...
def register():
    if request.method == 'POST':
        ...
        is_admin = 1 if username == 'adminroot' else 0  # adminroot인 경우 관리자로 설정
...
def dashboard():
    ...
    # 검색어가 있는 경우 필터링된 상품 조회
    if search_query:
        cursor.execute("""
            SELECT p.*, u.username as seller_username, u.is_blocked as seller_is_blocked 
            FROM product p 
        ...
        # 검색 후 결과를 보여주고 URL에서 search 파라미터 제거
        return redirect(url_for('dashboard'))
    else:
        # 모든 상품 조회
        cursor.execute("""
    ...
    return render_template('dashboard.html', products=products, user=current_user, search_query=search_query)
...
def new_product():
    ...
    db = get_db()
    cursor = db.cursor()
    cursor.execute("SELECT warning_count, is_blocked FROM user WHERE id = ?", (session['user_id'],))
    user = cursor.fetchone()
    
    if user['warning_count'] >= 3 or user['is_blocked']:
        flash('경고 누적으로 상품 등록이 불가능합니다. 관리자에게 문의 바랍니다.')
        return redirect(url_for('dashboard'))
...
def view_product(product_id):
    ...
    return render_template('view_product.html', product=product, user=current_user)
...
def admin_panel():
    ...
    # 채팅 로그 조회
    cursor.execute("SELECT * FROM chat_log ORDER BY timestamp DESC")
    chat_logs = cursor.fetchall()
    
    return render_template('admin_panel.html', users=users, products=products, chat_logs=chat_logs)

@app.route('/adminpage/reset_warnings/<user_id>', methods=['POST'])
def reset_warnings(user_id):
    ...
    cursor.execute("UPDATE user SET warning_count = 0, is_blocked = 0 WHERE id = ?", (user_id,))
    db.commit()
    flash('경고가 초기화되었습니다.')
    return redirect(url_for('admin_panel'))

@app.route('/adminpage/delete_product/<product_id>', methods=['POST'])
...
@app.route('/adminpage/toggle_block/<user_id>', methods=['POST'])
def toggle_block(user_id):
    ...
    new_status = 0 if current_status['is_blocked'] else 1
    
    cursor.execute("UPDATE user SET is_blocked = ? WHERE id = ?", (new_status, user_id))
    db.commit()
    
    action = "차단" if new_status else "차단 해제"
    flash(f'사용자가 {action}되었습니다.')
    return redirect(url_for('admin_panel'))
